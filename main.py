...
def print_help():
    ...
    print(
        "  status                            - Shows the status of all nodes in the network."
    )
    print(
        "  visualize [filename.png]          - Generates a PNG image of the network topology."
    )
    print(
        "  path <from> <to>                  - Finds and displays the fastest path between two nodes."
    )
    print(
        "  route <from> <to> <payload>       - Routes a message from a source to a destination node."
    )
    print(
        "  set_latency <from> <to> <ms>      - Changes the latency of a link between two nodes."
    )
...
def print_status(network):
    ...
    for node in sorted(network.nodes.values(), key=lambda n: n.name):
        status_colored = (
            "\033[92mONLINE\033[0m" if node.is_active else "\033[91mOFFLINE\033[0m"
        )
        print(f"Node: {node.name:<25} | Status: {status_colored}")
        neighbor_strings = []
        for neighbor, latency in sorted(
            node.neighbors.items(), key=lambda item: item[0].name
        ):
        ...
        print(
            f"  - Neighbors: {', '.join(neighbor_strings) if neighbor_strings else 'None'}"
        )
    print("----------------------\n")
        ...
        while True:
            try:
                ...
                if command in ["exit", "quit"]:
                    ...
                    if len(cli_args) != 1:
                        print(f"Usage: {command} <node_name>")
                        continue
                    ...
                    if node:
                        ...
                        print(f"Error: Node '{cli_args[0]}' not found.")
                elif command == "path":
                    if len(cli_args) != 2:
                    ...
                    from_node, to_node = network.get_node_by_name(
                        cli_args[0]
                    ), network.get_node_by_name(cli_args[1])
                    ...
                    if path:
                        print(
                            f"Fastest Path: {' -> '.join([n.name for n in path])} (Latency: {latency}ms)"
                        )
                    else:
                        print(
                            f"No path found between '{from_node.name}' and '{to_node.name}'."
                        )
                elif command == "set_latency":
                    if len(cli_args) != 3:
                        print("Usage: set_latency <from> <to> <ms>")
                        continue
                    try:
                        latency = int(cli_args[2])
                        if not network.set_link_latency(
                            cli_args[0], cli_args[1], latency
                        ):
                    ...
                    if len(cli_args) < 3:
                        print("Usage: route <from> <to> <payload>")
                        continue
                    from_node, to_node = network.get_node_by_name(
                        cli_args[0]
                    ), network.get_node_by_name(cli_args[1])
