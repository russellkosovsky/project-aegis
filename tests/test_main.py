...
@patch('main.yaml.safe_load', return_value={'nodes': [], 'links': []}) # Return dummy config data
...
def test_cli_status_command(mock_stdout, mock_input, mock_create_net, mock_open, mock_yaml, mock_validate):
    """
...
@patch('sys.stdout', new_callable=StringIO)
def test_cli_path_command(mock_stdout, mock_input, mock_create_net, mock_open, mock_yaml, mock_validate):
